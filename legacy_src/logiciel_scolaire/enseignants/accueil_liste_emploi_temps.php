...
var affiche_classe_temps=$("#classe").val();
if (affiche_classe_temps!==""){
	$('#loader').show();//on affiche le chargement quand l'utilisateur tape sur le clavier
	$.post('post.php',{affiche_classe_temps:affiche_classe_temps},function(data){
       ...
       <?php include "php/menu_enseignants.php"; ?>  
        ...
        <div id="page-wrapper" >
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                     <h2><?php echo $enseignants; //variable comportant le nom de la section ?> /Emploi de temps </h2>   
                    </div>
                </div>              
                 <!-- /. ROW  -->
                  <hr />
...
if(isset($_GET['1l_1c']) and isset($_GET['id_classe'])  )
{
	...
	for($i=1; $i<=10; $i++)
	{$v=0;
		if (mysql_query("update emploi_temps set heure='".$_GET[''.$i.'l_1c']."',lundi='".$_GET[''.$i.'l_2c']."',mardi='".$_GET[''.$i.'l_3c']."',mercredi='".$_GET[''.$i.'l_4c']."',jeudi='".$_GET[''.$i.'l_5c']."',vendredi='".$_GET[''.$i.'l_6c']."',samedi='".$_GET[''.$i.'l_7c']."', date_enre_emploi_temps='".time()."' where id_classe='".$_GET['id_classe']."' and numero_horaire='".$i."'"))
		{ $v=1; }
		else {$v=0;}
	}
	if($v==1){
