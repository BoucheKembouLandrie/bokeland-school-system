...
require('fpdf.php');
$visible="non";
if ( isset($_GET['numero_sequence']) and isset($_GET['id_eleve']) ){
	//fin de la recupération des données
	$id_eleve=$_GET['id_eleve'];
	$numero_sequence=$_GET['numero_sequence'];
...
$matiere=strtoupperfr($mat['nom_matiere']);
$note_seq = array();
...
$req_note_tous=mysqli_query($mysqli, "select *from note where id_sequence=$id_sequence order by id_sequence asc");
$note_seq_tous = array();
$tempo1 = 0; $ii = 0; $jj=0;
...
while($note_tous=mysqli_fetch_array($req_nonbre_classe2)){
	$ii++;
	$req_not=mysqli_query($mysqli, "select AVG ($matiere as moy) from note where id_sequence=$id_sequence and id_eleve=$id_eleve ");
