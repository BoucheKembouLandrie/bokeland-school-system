...
{	$id_personnel=$_GET['id_personnel'];
	...
	//on recuper les donnees passees par url
	 ...
	 $prenom=utf8_decode($_GET['prenom']);
	  ...
	  $ele=mysqli_fetch_array(mysqli_query($mysqli, "select *from eleve where matricule_eleve='$matricule'"));
	  $id=$ele['id_eleve'];
	  $idc=$ele['id_classe'];
...
////////////-------------------------------------------------------------------------------------------------------------------------------
$pdf->Image('logo.png', $x=90, $y=5,30,30); //Image('$nom', $bordure gauche, $bordurehaut,largeur,hauteur)
$pdf->Ln(10);
$pdf->Cell(40);
...
$dossier_source2='D:/'; //c'est dans ce repertoire qui contiendra l'acte à insérer.
	...
	if (!is_dir($dossier_source2)){
			mkdir($dossier_source2);}
//----------------------------------fin de creation du repertoire de l'eleve-----------------------------------------------------------------------------
	$dossier_source.="liste appel ";
	$dossier_source.=$date_jour;
	 $dossier_source.=".pdf";
	 
	 $dossier_source2.="liste appel ";
