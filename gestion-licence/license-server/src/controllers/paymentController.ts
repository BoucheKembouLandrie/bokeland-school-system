...
export const createPayment = async (req: Request, res: Response) => {
    ...
    try {
        ...
        // Update client
        // KEY RULE: If client is currently EXPIRED (regardless of what date is stored),
        // always start fresh from TODAY. Never accumulate on top of an old date.
        const now = new Date();
        const clientIsExpired = client.status === 'EXPIRED';
        const baseDate = clientIsExpired ? now : (() => {
