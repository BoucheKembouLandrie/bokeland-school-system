...
$a="index.jpg";
$pdf->Image($a, $x=157, $y=49,30,30); //Image('$nom', $bordure gauche, $bordurehaut,largeur,hauteur)
//------------------------------------------------------------------------------------------------------------------------------------
$pdf->Ln(4);$hauteur=$hauteur+4;
$pdf->Cell(5);
...
//-----------------------------------------------------------------------------------------------------------------
$req_mat_groupe=mysql_query("select *from matiere M, groupe G where G.id_classe=$id_vrai_classe and G.numero_groupe=$j and M.id_matiere=G.id_matiere order by nom_matiere asc");
...
while($mat=mysql_fetch_array($req_mat_groupe)){
//--------selection de la note de l'élève et du profetteur
$note=mysql_fetch_array(mysql_query("select *from note where id_sequence=$id_sequence and id_eleve=$id_eleve"));
//$prof=mysql_fetch_array(mysql_query("select *from personnel where"));
//-------------selection du coef de chaque matiere
$coef=mysql_fetch_array(mysql_query("select *from affectation_matiere where id_matiere='".$mat['id_matiere']."' and id_classe=$id_vrai_classe"));

//on cherche les les informations du professeur qui eseigne la matiere
$prof=mysql_fetch_array(mysql_query("select *from affectation_matiere A, personnel P where A.id_enseignant=P.id_personnel and  id_matiere='".$mat['id_matiere']."' and id_classe=$id_vrai_classe"));
...
{$pdf->Cell(0,5,utf8_decode($note[''.majuscule_sans_axcent($mat['nom_matiere']).'']),'1','C');}
...
$rang=mysql_fetch_array(mysql_query("SELECT COUNT(*)+1 AS Rang from note 
...
{$pdf->Cell(0,5,utf8_decode($rang['Rang']),'1','C'); }
//---------------------------------notes min / max

$min=mysql_fetch_array(mysql_query("SELECT min($matiere) AS mmin FROM note N where $matiere>0 and N.id_sequence=$id_sequence and N.id_classe=$id_vrai_classe"));
$max=mysql_fetch_array(mysql_query("SELECT max($matiere) AS mmax FROM note N where $matiere>0 and N.id_sequence=$id_sequence and N.id_classe=$id_vrai_classe"));
$pdf->Ln(0);
$pdf->Cell(111);
$pdf->SetFont('Times','',8);
...
$pdf->Cell(0,5,utf8_decode('['.chiffre_apres_virgule($mini,2)."--".chiffre_apres_virgule($maxi,2).']'),'1','C');


//---------------------------------pourcentage de reussite
//calcul du pourcentage de reussite de la matière

$nombre_moyenne_matiere=mysql_num_rows(mysql_query("select *from note where id_sequence=$id_sequence and id_classe=$id_vrai_classe and $matiere>=10"));
...
$pdf->Cell(130);
$pdf->SetFont('Times','',8);
$pdf->Cell(0,5,utf8_decode(chiffre_apres_virgule($poucentage, 2))." %",'1','C');
//---------------------------------Appreciation
$pdf->Ln(0);
$pdf->Cell(147);
$pdf->SetFont('Times','',8);

$pdf->Cell(0,5,utf8_decode(appreciation($note[''.majuscule_sans_axcent($mat['nom_matiere']).''])),'1','C');
...
$reqi=mysql_query("select *from eleve where id_classe=$id_vrai_classe and visible_eleve!='non'");

  $tab_eleve_groupe=array(); $k=0;
  while ($t111=mysql_fetch_array($reqi))
  {  $MOY=0; $tab_point_groupe_chaque_eleve=array();
  //cette requette permet d'avoir les notes de de group de tous les élèves..................
  	$req_mat_g=mysql_query("select *from matiere M, groupe G where G.id_classe=$id_vrai_classe and G.numero_groupe=$j and M.id_matiere=G.id_matiere order by nom_matiere asc");	 
	//..................................................
  $note_eleve_groupe=0;$coef_eleve_groupe=0; $k++; //ces variable servent a sommer successivement les notes et les coef de tous les élèves
	  while($t222=mysql_fetch_array($req_mat_g))
  { 
  
  $note_el=mysql_fetch_array(mysql_query("select *from note where id_sequence=$id_sequence and id_classe=$id_vrai_classe and id_eleve='".$t111['id_eleve']."'")); 
   $coef_el=mysql_fetch_array(mysql_query("select *from affectation_matiere where id_matiere='".$t222['id_matiere']."' and id_classe=$id_vrai_classe"));
...
$pdf->Cell(111);
$pdf->SetFont('Times','B',8);
$pdf->Cell(19,5,utf8_decode('['.chiffre_apres_virgule(min($tab_eleve_groupe),2)."--".chiffre_apres_virgule(max($tab_eleve_groupe),2).']'),'1','C');
//---------------------------------pourcentage de reussite
$pdf->Ln(0);
$pdf->Cell(130);
$pdf->SetFont('Times','B',9);
...
$req_chaque_eleve=mysql_query("select *from eleve where id_classe=$id_vrai_classe and visible_eleve!='non'");
 $k=0;
  while ($t111_chaque_eleve=mysql_fetch_array($req_chaque_eleve))
  {  $somme_note_chaque_eleve=0; $somme_coef_chaque_eleve=0;$k++;
  //cette requette permet d'avoir les notes de de group de tous les élèves..................
  	$req_mat_chaque_eleve=mysql_query("select *from matiere M, groupe G where G.id_classe=$id_vrai_classe and M.id_matiere=G.id_matiere order by nom_matiere asc");	 
	//..................................................
	  while($t222_chaque_eleve=mysql_fetch_array($req_mat_chaque_eleve))
  {   
  $note_chaque_eleve=mysql_fetch_array(mysql_query("select *from note where id_sequence=$id_sequence and id_classe=$id_vrai_classe and id_eleve='".$t111_chaque_eleve['id_eleve']."'")); 
  
   $coef_chaque_eleve=mysql_fetch_array(mysql_query("select *from affectation_matiere where id_matiere='".$t222_chaque_eleve['id_matiere']."' and id_classe=$id_vrai_classe"));
...
$pdf->Image("sequence6.png", $x=10, $y=$hauteur,58,40); //Image('$nom', $bordure gauche, $bordurehaut,largeur,hauteur)
