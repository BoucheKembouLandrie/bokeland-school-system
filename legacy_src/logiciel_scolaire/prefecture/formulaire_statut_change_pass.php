...
<div class="title"><h2 style="color:#03C;">Verifications</h2></div>
    ...
    <input class="large" type="text" name="mo1" required placeholder="mot de passe"/></div>
    <div class="element-input">
    <label class="title">Reentrer ce mot de passe<span class="required"></span></label>
    <input class="large" type="text" name="mo2" required placeholder="meme mot de passe"/></div>
...
if($mo1==$mo2){
 	  if(mysqli_query($mysqli, 'update statut_personnel set pass_statut_personnel="'.$mo1.'" where id_statut_personnel="'.$id_statut_personnel.'"'))

    { echo'<script language="javascript"> alert("Modification effectuée!!! ")</script>'; 
	echo'<script language="javascript"> document.location.href="statut_change_pass.php";</script>';
  exit();}
