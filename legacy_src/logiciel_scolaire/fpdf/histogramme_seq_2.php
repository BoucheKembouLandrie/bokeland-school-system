...
for($i=1; $i<=2; $i++){
	$tableauAnnees[] = 'Séq ' . $i;
	$tableauNombreNotes[] =chiffre_apres_virgule(calcul_note_sequence($id_eleve,$i),2);
}
...
// Le titre
$graph->title->Set("Histogramme des notes de ".utf8_decode($nom_eleve)." ".utf8_decode($prenom)."");
...
//repertoire dans lequel sera ranger l'histogramme de l'élève
			...
			$graph->Stroke(''.$dossier_source.'/histogramme sequence 1.png');
			$graph->Stroke(''.$dossier_source2.'/histogramme sequence 1.png');
