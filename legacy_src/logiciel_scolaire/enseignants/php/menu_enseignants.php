...
<?php if (!isset($_SESSION['id_membre_ens']))
	{
                    ...
                    <img src="../photo_profil_du_personnel/<?php echo $a['photo_personnel']; ?>" height="250" width="250" class="user-image img-responsive"/><?php echo '<a style="color:FFFFF;">Bienvenue '.sexe($a['sex_personnel'])." ".$a['nom_personnel']." ".$a['prenom_personnel'].'</a>'; ?> 
                      ...
                      <li  >
                        ...
                        <ul class="nav nav-second-level">
                            ...
                            <li>
                                <a href="accueil_liste_emploi_temps.php">Emploi de temps</a>
                            </li>
                        </ul>
                      </li>
                    </li>
                        ...
                        <ul class="nav nav-second-level">
                                 <?php for($i=1; $i<=6; $i++){ ?>
                                           <?php $etat_seq=mysqli_fetch_array(mysqli_query($mysqli, "select etat_sequence from sequence where id_sequence=$i")); ?>
                                            ...
                                            <?php if($etat_seq['etat_sequence']==1) {?>
                                     <li>
                                		<a href="sequence_<?php echo $i; ?>.php">Sequence <?php echo $i; ?></a>
                            		</li>
                                            <?php } ?>
                                        <?php } ?>
                                        
                  <?php $nrbe_classe_titulaire=mysqli_num_rows(mysqli_query($mysqli, "select * from classe where id_enseignant_titulaire=$id1")); ?>       
                               <?php if($nrbe_classe_titulaire!==0) {?>
                            <li>
                                <a href="#" onclick="verification_bull(<?php echo $id; ?>);">Bulletins</a>
                            </li>
                            <?php } ?>
