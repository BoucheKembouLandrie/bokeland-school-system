...
$pdf->Cell(65,7,utf8_decode("BULLETIN ANNUELLE"),'1','C');
//------------------------------------------------------------------------------------------------------------------------------------
$pdf->Ln(7);$hauteur=$hauteur+7;
...
$pdf->Cell(85,7,utf8_decode($nom),'C');
...
$pdf->Image($photo_eleve, $x=157, $y=49,30,30); //Image('$nom', $bordure gauche, $bordurehaut,largeur,hauteur)
//------------------------------------------------------------------------------------------------------------------------------------
$pdf->Ln(4);$hauteur=$hauteur+4;
$pdf->Cell(5);
...
$pdf->Cell(40);
$pdf->SetFont('Times','B',9);
$pdf->Cell(0,7,utf8_decode("Trim1"),'1','C');
$pdf->Ln(0);
$pdf->Cell(50);
$pdf->SetFont('Times','I',6);
...
$pdf->Cell(50);
$pdf->SetFont('Times','B',9);
$pdf->Cell(0,7,utf8_decode("Trim2"),'1','C');
$pdf->Ln(0);
$pdf->Cell(50);
$pdf->SetFont('Times','I',6);
$pdf->Cell(0,3,utf8_decode("Avg/20"),'C');
...
$pdf->Cell(60);
$pdf->SetFont('Times','B',9);
$pdf->Cell(0,7,utf8_decode("Trim3"),'1','C');
$pdf->Ln(0);
$pdf->Cell(60);
$pdf->SetFont('Times','I',6);
$pdf->Cell(0,3,utf8_decode("Avg/20"),'C');
...
while($note=mysqli_fetch_array($req_note)){
	$note_seq[$t] =  $note[''.majuscule($mat['nom_matiere']).''];
	$t++;
	}
	$seq_1=$note_seq[0];
	$seq_2=$note_seq[1];
	...
	$seq_5=$note_seq[4];
	$seq_6=$note_seq[5];
	$trim1=($seq_1+$seq_2)/2; $trim2=($seq_3+$seq_4)/2; $trim3=($seq_5+$seq_6)/2;
	$trim=($trim1+$trim2+$trim3)/3;
	
	//--------selection de la note de tous les élèves 
	
$req_note_tous=mysqli_query($mysqli, "select *from note where id_trimestre=$trim order by id_sequence asc");
$note_seq_tous = array();
$tempo1 = 0; $ii = 0; $jj=0;
...
while($note_tous=mysqli_fetch_array($req_nonbre_classe2)){
	$ii++;
	$req_not=mysqli_query($mysqli, "select AVG ($matiere as moy) from note where id_trimestre=$trim and id_eleve=$id_eleve ");
 ...
 {
	 echo $valeur_chaque_eleve; if ($valeur_chaque_eleve>=10)$c_chaque_eleve_par_mat++;//on compte le nombre de moyenne du groupe
...
$pdf->Cell(40);
$pdf->SetFont('Times','B',8);
if($note[''.strtoupperfr($mat['nom_matiere']).'']<0)
{$pdf->Cell(0,5,utf8_decode(""),'1','C');}
else
{$pdf->Cell(0,5,utf8_decode($trim1),'1','C');}
//---------------------------------moyennes trim 2
$pdf->Ln(0);
$pdf->Cell(50);
$pdf->SetFont('Times','B',8);
...
{$pdf->Cell(0,5,utf8_decode($trim2),'1','C');}
...
{$pdf->Cell(0,5,utf8_decode($trim3),'1','C');}
...
$pdf->Cell(130);
$pdf->SetFont('Times','',8);
$pdf->Cell(0,5,utf8_decode($taux_reussite_matiere)." %",'1','C');
//---------------------------------Appreciation
$pdf->Ln(0);
$pdf->Cell(147);
$pdf->SetFont('Times','',8);

$pdf->Cell(0,5,utf8_decode (appreciation($note_matiere)),'1','C');
//$pdf->Cell(0,5,utf8_decode($note_matiere=$trim),'1','C');
...
//-------------------------------------calcul du pourcentage et rang du groupe------------------------------------ 
 ...
 foreach($tab_eleve_groupe as $v)
 {if ($v>=10)$c++;//on compte le nombre de moyenne du groupe
 if($moyenne_groupe<$v)$r++;
	 }
	 $rang_groupe=$r+1;
...
$req_chaque_eleve=mysqli_query($mysqli, "select *from eleve where id_classe=$id_vrai_classe and visible_eleve!='non'");
 $k=0;
  ...
  {   $matiere = $t222_chaque_eleve['nom_matiere'];
  $note_chaque_eleve=mysqli_fetch_array(mysqli_query($mysqli, "select AVG($matiere) as  moy_trim from note where id_trimestre=$id_sequence and id_classe=$id_vrai_classe and id_eleve='".$t111_chaque_eleve['id_eleve']."'")); 
   ...
   $coef_chaque_eleve=mysqli_fetch_array(mysqli_query($mysqli, "select *from affectation_matiere where id_matiere='".$t222_chaque_eleve['id_matiere']."' and id_classe=$id_vrai_classe"));
		...
		if($note_chaque_eleve['moy_trim']<0){}
		 else
		{$somme_coef_chaque_eleve=$somme_coef_chaque_eleve+ $coef_chaque_eleve['coef_matiere'];} 
		 
		 if($note_chaque_eleve['moy_trim']<0){}
		 else
		{$somme_note_chaque_eleve=$somme_note_chaque_eleve+$note_chaque_eleve['moy_trim']*$coef_chaque_eleve['coef_matiere'];}
...
$dossier_source3='C:/'; //c'est dans ce repertoire qui contiendra l'acte à insérer.
	$dossier_source3= sous_repertoire($dossier_source3, "Bulletins annuelle"); // lien du premier sous repertoire
...
if($verification==1) {echo '<script language="javascript"> alert("Les bulletins ont bien été générés. Ils se trouvent dans votre disque '.$disque.' dans le repertoire: Bulletins annee/'.$classe.'");</script>';}
