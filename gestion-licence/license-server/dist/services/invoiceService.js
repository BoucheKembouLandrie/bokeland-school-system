...
const generateInvoice = (data) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            const invoicePath = path_1.default.join(invoicesDir, `${data.invoiceNumber}.pdf`);
            ...
            generateTableRow(doc, iInvoiceTableTop, 'Description', 'Quantité', 'Prix Unitaire', 'Total');
            generateHr(doc, iInvoiceTableTop + 20);
            doc.font('Helvetica');
            // --- Table Content ---
            const iInvoiceTableContent = 355;
            generateTableRow(doc, iInvoiceTableContent, data.description, '1', formatCurrency(data.amount), formatCurrency(data.amount));
            generateHr(doc, iInvoiceTableContent + 20);
            // --- Total ---
            const subtotalPosition = iInvoiceTableContent + 40;
            doc.font('Helvetica-Bold');
            generateTableRow(doc, subtotalPosition, '', '', 'TOTAL', formatCurrency(data.amount));
            // --- Footer ---
