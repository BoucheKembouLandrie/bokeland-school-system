...
var numero_sequence=3;
var numero_trimestre=2;
if (affiche_eleve!==""){
	$('#loader').show();//on affiche le chargement quand l'utilisateur tape sur le clavier
...
<div class="row">
       ...
       <!-- barre  -->
                ...
                <div class="row" >
                    ...
                    <h4 style="color:#03F">Notes séquence N°3</h4>
  ...
  $jjj=0; $verification=0;
		while ($tab_mat=mysqli_fetch_array($req_mat)){ $jjj++;
		       ...
		       for($ii=1; $ii<=$compteur; $ii++)
				{
					if(empty($_GET['note_'.$ii.'L_'.$jjj.'C']))$tab_note_mat[$ii][$jjj]=-1;
					else $tab_note_mat[$ii][$jjj]=$_GET['note_'.$ii.'L_'.$jjj.'C'];
				}  
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------
	 //3............On cree des nouvelle propriétés a la table note............ces propriete sont les matiere de la classe
	   ...
	   mysqli_query($mysqli,"ALTER TABLE note ADD $nom_matiere  FLOAT NOT NULL DEFAULT -1 AFTER date_enre_note ");
		 }
		 ...
		 $nom_mat=mysqli_fetch_array( mysqli_query($mysqli,"select *from matiere where id_matiere=$v and id_classe=$id_classe"));
	 		...
	 		{ //---------------------------------------------------------------------------------
				...
				{ //dans ce cas, on fait un insert car c'est la premiere fois
		 		if (mysqli_query($mysqli,"insert into note (id_eleve, id_sequence, id_trimestre, id_classe, date_enre_note, $nom_matiere) value('".$tab_ID_ele[$in]."','".$numero_sequence."','".$numero_trimestre."','".$id_classe."','".time()."','".$tab_note_mat[$in][$c]."')"))
		 		{$verification=1;}
		 		else
		 		{$verification=0;}
				}
				...
				{ //si ($nombre_note!==0) on fait un update
					if (mysqli_query($mysqli,"update note set  id_eleve='".$tab_ID_ele[$in]."', id_sequence='".$numero_sequence."', id_trimestre='".$numero_trimestre."', id_classe='".$id_classe."',date_enre_note='".time()."', $nom_matiere='".$tab_note_mat[$in][$c]."' where  id_eleve='".$tab_ID_ele[$in]."' and id_sequence='".$numero_sequence."' and  id_trimestre='".$numero_trimestre."' "))
		 		{$verification=1;}
		 		else
		 		{$verification=0;} 
				}
