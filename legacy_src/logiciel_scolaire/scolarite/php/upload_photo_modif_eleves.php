...
$nom_etablissement=majuscule($et['nom_etablissement']);
	 $nom=$_POST['nom'];
	 $prenom=$_POST['prenom'];
	 $id_vrai_classe= $_POST['id_classe'];
	 $photo_eleve=$_POST['photo_eleve'];
	  ...
	  $nom_eleve= majuscule($nom); $nom_eleve.=" "; $nom_eleve.=$prenom;
	  $id_eleve=$_POST['id_eleve'];
	  
	


$dossier_source='../'; //c'est ce repertoire qui contiendra l'acte à insérer.
	...
	//$dossier_source.=$photo_eleve; //lien final de la photo de l'élèves
...
$dossier_source2='D:/'; //c'est dans ce repertoire qui contiendra l'acte à insérer.
	...
	if (!is_dir($dossier_source2)){
			mkdir($dossier_source2);}
		//		$dossier_source2.=$photo_eleve; //lien final de la photo de l'élèves
//----------------------------------fin de creation du repertoire de l'eleve-----------------------------------------------------------------------------
      ...
      if($infosImg[2] >= 1 && $infosImg[2] <= 14)
      {
        ...
        if(($infosImg[0] <= WIDTH_MAX) && ($infosImg[1] <= HEIGHT_MAX) && (filesize($_FILES['logo']['tmp_name']) <= MAX_SIZE))
        {
            ...
            && UPLOAD_ERR_OK === $_FILES['logo']['error'])
          {
            ...
            if(move_uploaded_file($_FILES['logo']['tmp_name'], TARGET.$photo_eleve))
            {
              /*$message = '<script language="javascript"> alert("Enregistrement reussi")</script> !';*/
			  if(copy(TARGET.$photo_eleve, TARGET2.$photo_eleve))$verification=1;
			}
            else
            {
