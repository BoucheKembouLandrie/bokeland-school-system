...
$tableauAnnees=array();
$tableauNombreNotes=array();
$tableauNombreNotesann=array();

for($i=1; $i<=$id_sequence; $i++){
	$tableauNombreNotes[] =chiffre_apres_virgule(calcul_note_sequence($mysqli, $id_eleve, $i),2);
}
for($j=1; $j<=3; $j++){
	$tableauAnnees[] = 'trim '.$j;
	}
	$tableauNombreNotesann[0]=($tableauNombreNotes[0]+$tableauNombreNotes[1])/2;
	$tableauNombreNotesann[1]=($tableauNombreNotes[2]+$tableauNombreNotes[3])/2;
	$tableauNombreNotesann[2]=($tableauNombreNotes[4]+$tableauNombreNotes[5])/2;
...
$bplot = new BarPlot($tableauNombreNotesann);
...
$graph->xaxis->title->Set(utf8_decode("annee"));
...
$histogramme1= sous_repertoire($histogramme1, "histogrammes"); //lien du qutrieme sous repertoire
	if (!is_dir($histogramme1)){
			mkdir($histogramme1);}
$histogramme1.='/histogramme_annee.png';
...
$histogramme2= sous_repertoire($histogramme2, "histogrammes"); //lien du qutrieme sous repertoire
	if (!is_dir($histogramme2)){
			mkdir($histogramme2);}
$histogramme2.='/histogramme_annee.png';
